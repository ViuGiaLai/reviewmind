...
export function AutoFixPlanner({
    ...
    try {
      ...
      if (resp.ok) {
        ...
        for (const item of (data.items || [])) {
          if (!item.applied && item.confidence >= 90) {
            safeIds.add(item.id);
          }
        }
  ...
  if (mode === "safe") {
    visibleSuggestions = pendingSuggestions.filter(s => s.confidence >= 90);
  }
        ...
        <div className="modal-content autofix-planner-modal" style={{ maxWidth: 600 }}>
          ...
          <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ padding: 16, background: 'var(--success-bg, rgba(46, 204, 113, 0.1))', color: 'var(--success)', borderRadius: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
              <CheckCircle size={24} />
              <div>
                <strong>Successfully applied {summary.applied} fixes</strong>
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
                <div 
                  ...
                  onClick={() => {
                    setMode("safe");
                    const safeIds = pendingSuggestions.filter(s => s.confidence >= 90).map(s => s.id);
              ...
              <div style={{ background: 'var(--bg-elevated)', padding: 16, borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <strong>{visibleSuggestions.length} lỗi có thể sửa tự động</strong> trong chế độ <em>{mode === 'safe' ? 'Safe Fix' : mode === 'selected' ? 'Selected' : 'Smart Fix'}</em>.
                  <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: 4 }}>
                    {pendingSuggestions.length - visibleSuggestions.length} lỗi cần bạn xác nhận hoặc tự sửa tay.
                  </div>
                </div>
                {mode === "selected" && (
                  <button className="btn-sm outline" onClick={() => {
                    ...
                    {selectedIds.size === visibleSuggestions.length ? 'Deselect All' : 'Select All'}
                  </button>
                )}
              </div>
                    ...
                    <div key={s.id} className="autofix-item" style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                          ...
                          {isSafe && <span className="ss-tag pack" style={{ padding: '2px 6px', fontSize: '0.7rem' }}>Safe</span>}
                        </div>
